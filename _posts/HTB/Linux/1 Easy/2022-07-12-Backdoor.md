---
description: >-
  Backdoor HTB Machine
title: Backdoor (Medium)                # Add title here
date: 2022-07-12 08:00:00 -0600                           # Change the date to match completion date
categories: [HackTheBox, Linux - Easy]                     # Change Templates to Writeup
tags: [hackthebox, wordpress, lfi, rce, proc-cmdline, gdbserver, screen linux command]     # TAG names should always be lowercase; replace template with writeup, and add relevant tags
show_image_post: true                                    # Change this to true
image: /assets/img/icons/Backdoor.png                # Add infocard image here for post preview image
---
...
Now that we know that there is a gdbserver[^gdbserver-rce] running, we proceed to search for exploits for this service and we found the following: 
...
Get SUID[^suid-screen] by running following command:
...
So, the last command to escalate privileges is:
...
[gdbserver](https://www.exploit-db.com/exploits/50539)

[^gdbserver-rce]: GDBServer prior to 9.2 vulnerable to RCE
[^suid-screen]: Screen SUID Privilege Escalation